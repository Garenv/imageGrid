describe('user visits the login page', { testIsolation: false }, () => {
    ...
    describe('user attempts to login with', () => {
        ...
        context("invalid credentials", () => {
            let name = "GoingToBeLocked"
            let email = "Locked@hotmail.com"

            before(() => {
                cy.deleteUser(email)
                cy.createUser(name, email, "Password12345@!", "lockedAccountIp")
            });

            [
                ["NotARealEmail@gmail.com", "NotARealPassword1234", 'These credentials do not match our records.✖'],
                [name, "NotARealPassword1234", 'These credentials do not match our records.✖'],
        ...
        context('valid credentials', () => {
            ...
            context("valid credentials", () => {
                ...
                it(`should redirect to the grid`, () => {
                    ...
                    cy.contains('.MuiButtonBase-root', 'Profile').click()
                    cy.get('[data-cy="profile-name"]').then((element) => {
                        expect(element.text()).eq(faker.name)
                    })
                    cy.get('[data-cy="profile-email"]').then((element) => {
